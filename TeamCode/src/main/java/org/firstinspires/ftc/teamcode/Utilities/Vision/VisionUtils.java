...
public class VisionUtils {
    ...
    public static int getMaxIndexArr(Rect[] rects, RECT_OPTION option){
        int alpha_index = 0;
        double max = Integer.MIN_VALUE;
        double cur = 0;
     ...
     * @return
     */
    public static int getMaxIndexRotated(RotatedRect[] rects, RECT_OPTION option){
        int alpha_index = 0;
        double max = Integer.MIN_VALUE;
        double cur = 0;
        for (int i=0; i < rects.length; i++){
            ...
            switch (option){
                case X:
                    cur = rects[i].center.x;
                    break;

                case Y:
                    cur = rects[i].center.y;
                    break;

                case WIDTH:
                    cur = rects[i].size.width;
                    break;

                case HEIGHT:
                    cur = rects[i].size.height;
                    break;

                case AREA:
                    cur = rects[i].size.width * rects[i].size.height;
                    break;
